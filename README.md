...
ranks: bounded rollout, reward/verifier, replayable records,
staleness-aware experiences, a synchronized learner step, and checkpoint
publication.

The report describes an asynchronous post-training system that keeps rollout
concurrency bounded, dispatches at sample/group granularity, pauses generation
at token boundaries, persists rollout state, masks stale tokens, and resumes
...
length-bias control, and speculative draft/verification. The project includes
an algorithm-level simulator so the same workload is compared with all of
these controls off and on:
...
plus a delta section. The virtual decode cost is an explicit model-independent
cost unit; it is useful for checking algorithm direction, while real GPU
throughput still needs the rented hardware. The same comparison is available
...
The real path can be added after the multi-GPU contract tests pass:

...
3. `rollout.py` separates the local backend from the optional SGLang HTTP
   backend. The adapter uses SGLang Omni's `/generate` contract and refuses
   text-only responses because RL needs token IDs and sampled-token logprobs.
...
Before a public release, the rented-GPU validation should connect a real SGLang
